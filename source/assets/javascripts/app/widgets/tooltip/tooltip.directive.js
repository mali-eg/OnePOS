(function(window, angular, undefined){
    ...
    var module = angular.module('onePOS.widgets');
    var $el = angular.element;
    var isDef = angular.isDefined;
    var $popovers = [];
    var globalId = 0;

    module.provider('opsPopover', function () {
        var defaults = {
            templateUrl: '/assets/javascripts/app/widgets/tooltip/tooltip.html',
            //theme: 'ops-popover-list-theme',
            plain: 'false',
            trigger: 'click',
            triggerPrevent: true,
            angularEvent: '',
    ...
    module.directive('opsPopover', ['opsPopover','$rootScope','$timeout','$templateCache','$q','$http','$compile','$document','$parse',
        function(opsPopover, $rootScope, $timeout, $templateCache, $q, $http, $compile, $document, $parse) {
            return {
                restrict: 'A',
                scope: {
                    tooltipTitle: '@?',
                ...
                link: function(scope, elm, attrs) {
                    var defaults = opsPopover.getDefaults();

                    var options = {
                        template: /*attrs.opsPopoverTemplate || */defaults.templateUrl,
                        //theme: attrs.opsPopoverTheme || defaults.theme,
                        plain: toBoolean(attrs.opsPopoverPlain || defaults.plain),
                        trigger: attrs.opsPopoverTrigger || defaults.trigger,
                        triggerPrevent: attrs.opsPopoverTriggerPrevent || defaults.triggerPrevent,
                        angularEvent: attrs.opsPopoverAngularEvent || defaults.angularEvent,
                        scopeEvent: attrs.opsPopoverScopeEvent || defaults.scopeEvent,
                        container: attrs.opsPopoverContainer || defaults.container,
                        placement: attrs.opsPopoverPlacement || defaults.placement,
                        timeout: attrs.opsPopoverTimeout || defaults.timeout,
                        hideOnInsideClick: toBoolean(attrs.opsPopoverHideOnInsideClick || defaults.hideOnInsideClick),
                        hideOnOutsideClick: toBoolean(attrs.opsPopoverHideOnOutsideClick || defaults.hideOnOutsideClick),
                        hideOnButtonClick: toBoolean(attrs.opsPopoverHideOnButtonClick || defaults.hideOnButtonClick),
                        //TODO: onePos
                        closeBtn: toBoolean(attrs.opsPopoverCloseButton || defaults.closeBtn),
                        //TODO: ENDOF: onePos
                        mouseRelative: attrs.opsPopoverMouseRelative,
                        popupDelay: attrs.opsPopoverPopupDelay || defaults.popupDelay,
                        group: attrs.opsPopoverGroup
                    };
                    ...
                    var displayer_ = {
                        ...
                        display: function(delay, e) {
                            // Disable popover if ops-popover value is false
                            if ($parse(attrs.opsPopover)(scope) === false) {
                                return;
                            }
                            ...
                            if (options.group) {
                                $rootScope.$broadcast('ops:popover:hide', options.group);
                            }
                    ...
                    var $popover = $el('<div id="opspopover-' + globalId +'"></div>');
                    ...
                    $q.when(loadTemplate(options.template, options.plain)).then(function(template) {
                        ...
                        $popover
                            .addClass('ops-popover-' + placement_ + '-placement')
                            .addClass('ops-popover-' + align_ + '-align');
                        ...
                        scope.$on('ops:popover:hide', function(ev, group) {
