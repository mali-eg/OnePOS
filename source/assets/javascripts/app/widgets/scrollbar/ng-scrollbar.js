...
angular.module('onePOS.widgets')
    .directive('ngScrollbar', function () {
        return {
            ...
            link: function (scope, element) {
                ...
                scope.$on('recalculateNgScrollbars', function (event) {
                    setTimeout(function () {
                        recalculate();
                    }, 5);
                });

                scope.$on('scrollToNgScrollbars', function (event, offset) {
                    setTimeout(function () {
                        scrollTo(offset);
                    }, 5);
                });
    ...
    .service('ngScrollbar', function () {
        ...
        this.recalculate = function ($scope) {
            setTimeout(function () {
                $scope.$broadcast('recalculateNgScrollbars');
            }, 5);
        };
        this.scrollTo = function ($scope, event) {
            setTimeout(function () {
                $scope.$broadcast('scrollToNgScrollbars', event);
            }, 5);
        };
    });
