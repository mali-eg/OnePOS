...
// map global property to local variable.
    ...
    var opsSelect = angular.module('simplicity.select', []);

    /**
     * A service for configuration. the configuration is shared globally.
     */
    opsSelect.provider('opsConfig', function () {
    ...
    opsSelect.controller('opsSelectCtrl', function () {

        var self = this;

        // keyIdentifier and valueIdentifier are set by opsOption directive
        // used by opsSelect directive to retrieve key and value from each opsOption's child scope.
        self.keyIdentifier = null;
        self.valueIdentifier = null;

        self.isMultiple = false;

        // Should be override by opsSelect directive and called by opsOption directive when collection is changed.
    ...
    opsSelect.directive('opsSelect', ['$parse', '$document', '$timeout', 'opsConfig',
        function ($parse, $document, $timeout, opsConfig) {

            var DEFAULT_NONE_SELECTION = 'Select ..';

            var DROPDOWN_TOGGLE = '<button class="dropdown-toggle" type="button">' +
                '<span></span>' +
            ...
            var ACTIONS_BOX = '<div class="bs-actionsbox">' +
                '<div>' +
                '<button class="bs-select-all">SELECT ALL</button>' +
                '<button class="bs-deselect-all">DESELECT ALL</button>' +
                '</div>' +
                '</div>';
            return {
                restrict: 'ECA',
                require: ['ngModel', 'opsSelect'],
                controller: 'opsSelectCtrl',
                compile: function opsSelectCompile(tElement, tAttrs) {

                    tElement.addClass('btn-group');
                    // TODO: onePOS
                    // Default text for labels.
                    var creatingLabelNode = function () {
                        var content;
                        if (tAttrs.label) {
                            if (tAttrs.ngRequired) {
                                content = ("<label>" + tAttrs.label + "<span>*</span>" + "</label>");
                            } else {
                                content = ("<label>" + tAttrs.label + "</label>");
                            }
                        }
                        return content;
                    }
                    ...
                    var getDefaultNoneSelectionContent = function () {
                        ...
                        if (tAttrs.labelTpl) {
                            // use label-tpl attribute value.
                            content = jqLite(tAttrs.labelTpl);
                        } else if (tAttrs.label) {
                            // use label attribute value.
                            // content = document.createTextNode(tAttrs.label); // TODO: onePOS
                    ...
                    for (index = 0; index < length; index++) {
                        ...
                        if (liElement.hasClass('nya-bs-option') || liElement.attr('nya-bs-option')) {
                            ...
                            opsOptionValue = liElement.attr('value');
                            if (angular.isString(opsOptionValue) && opsOptionValue !== '') {
                                liElement.attr('data-value', opsOptionValue);
                    ...
                    if (tAttrs.liveSearch === 'true') {
                        ...
                        if (tAttrs.noSearchLabel) {
                            NO_SEARCH_RESULT = NO_SEARCH_RESULT.replace('NO SEARCH RESULT', tAttrs.noSearchLabel);
                        } else if (tAttrs.noSearchLabelTpl) {
                            NO_SEARCH_RESULT = NO_SEARCH_RESULT.replace('NO SEARCH RESULT', tAttrs.noSearchLabelTpl);
                        } else {
                            // set localized text
                    ...
                    tElement.append("<!-- LIB_ID:F2.10 -->"); // TODO: onePOS
                    tElement.append(creatingLabelNode()); // TODO: onePOS
                    tElement.addClass('fm-input fm-opsSelect'); // TODO: onePOS

                    tElement.append(dropdownToggle);
                    tElement.append(dropdownContainer);

                    return function opsSelectLink($scope, $element, $attrs, ctrls) {

                        var ngCtrl = ctrls[0],
                            opsSelectCtrl = ctrls[1],
                            liHeight,
                            isDisabled = false,
                            previousTabIndex,
                            valueExpFn,
                            valueExpGetter = $parse(opsSelectCtrl.valueExp),
                        ...
                        // for debug
                        opsSelectCtrl.setId($element.attr('id'));

                        if (isMultiple) {
                            opsSelectCtrl.isMultiple = true;
                         ...
                         * @param values
                         */
                        opsSelectCtrl.onCollectionChange = function (values) {
                            var valuesForSelect = [],
                                index,
                                length,
                            ...
                            if (Array.isArray(values) && values.length > 0) {
                                if (valueExpFn) {
                                    ...
                                    for (index = 0; index < values.length; index++) {
                                        if (opsSelectCtrl.valueIdentifier) {
                                            valuesForSelect.push(values[index][opsSelectCtrl.valueIdentifier]);
                                        } else if (opsSelectCtrl.keyIdentifier) {
                                            valuesForSelect.push(values[index][opsSelectCtrl.keyIdentifier]);
                                        }
                                    }
                        ...
                        dropdownMenu.on('click', function menuEventHandler(event) {
                            ...
                            var opsOptionNode = filterTarget(event.target, dropdownMenu[0], 'nya-bs-option'),
                                opsOption;

                            if (opsOptionNode !== null) {
                                opsOption = jqLite(opsOptionNode);
                                if (opsOption.hasClass('disabled')) {
                                    return;
                                }
                                selectOption(opsOption);
                            }
                        });
                        ...
                        dropdownToggle.on('click', function () {
                            ...
                            if ($attrs.liveSearch === 'true' && $element.hasClass('open')) {
                                ...
                                opsOptionNode = findFocus(true);
                                if (opsOptionNode) {
                                    dropdownMenu.children().removeClass('active');
                                    jqLite(opsOptionNode).addClass('active');
                                }
                            } else if ($element.hasClass('open')) {
                                opsOptionNode = findFocus(true);
                                if (opsOptionNode) {
                                    setFocus(opsOptionNode);
                                }
                            }
                        });
                        ...
                        ngCtrl.$render = function () {
                            ...
                            //console.log(opsSelectCtrl.id + ' render end');
                            updateButtonContent();
                        };
                        ...
                        $element.on('keydown', function (event) {
                            ...
                            if (toggleButton) {
                                ...
                                if (keyCode === 27) {
                                    ...
                                    event.stopPropagation();
                                    // up arrow key
                                    opsOptionNode = findNextFocus(event.target.parentNode, 'previousSibling');
                                    if (opsOptionNode) {
                                        setFocus(opsOptionNode);
                                    } else {
                                        opsOptionNode = findFocus(false);
                                        if (opsOptionNode) {
                                            setFocus(opsOptionNode);
                                        }
                                    }
                                    ...
                                    event.stopPropagation();
                                    // down arrow key
                                    opsOptionNode = findNextFocus(event.target.parentNode, 'nextSibling');
                                    if (opsOptionNode) {
                                        setFocus(opsOptionNode);
                                    } else {
                                        opsOptionNode = findFocus(true);
                                ...
                                if (keyCode === 27) {
                                    ...
                                    if (liElement) {
                                        opsOptionNode = findNextFocus(liElement[0], 'previousSibling');
                                        if (opsOptionNode) {
                                    ...
                                    liElement = findActive();
                                    if (liElement) {
                                        opsOptionNode = findNextFocus(liElement[0], 'nextSibling');
                                        if (opsOptionNode) {
                                            liElement.removeClass('active');
                                            jqLite(opsOptionNode).addClass('active');
                                        } else {
                        ...
                        function setAllOptions(selectAll) {
                            ...
                            if (liElements.length > 0) {
                                ...
                                for (var i = 0; i < liElements.length; i++) {
                                    var opsOption = jqLite(liElements[i]);
                                    if (opsOption.hasClass('disabled'))
                                        continue;
                                    ...
                                    value = getOptionValue(opsOption);
                                    ...
                                    if (typeof value !== 'undefined') {
                                        ...
                                        if (selectAll && index == -1) {
                                            ...
                                            opsOption.addClass('selected');
                                        } else if (!selectAll && index != -1) {
                                            // uncheck element
                                            viewValue.splice(index, 1);
                                            opsOption.removeClass('selected');
                                        }
                                    }
                                }
                        ...
                        /**
                         * select an option represented by opsOption argument. Get the option's value and update model.
                         * if isMultiple = true, doesn't close dropdown menu. otherwise close the menu.
                         * @param opsOption the jqLite wrapped `nya-bs-option` element.
                         */
                        function selectOption(opsOption) {
                            var value,
                                viewValue,
                                modelValue = ngCtrl.$modelValue,
                                index;
                        ...
                        /**
                         * get a value of current opsOption. according to different setting.
                         ...
                         * - if `nya-bs-option` directive is used to populate options and no other settings, use the valueIdentifier or keyIdentifier to retrieve value from scope of current opsOption.
                         * - if `nya-bs-option` class is used on static options. use literal value of the `value` attribute.
                         * @param opsOption a jqLite wrapped `nya-bs-option` element
                         */
                        function getOptionValue(opsOption) {
                            var scopeOfOption;
                            if (valueExpFn) {
                                // here we use the scope bound by ourselves in the nya-bs-option.
                                scopeOfOption = opsOption.data('isolateScope');
                                return valueExpFn(scopeOfOption);
                            } else {
                                if (opsSelectCtrl.valueIdentifier || opsSelectCtrl.keyIdentifier) {
                                    scopeOfOption = opsOption.data('isolateScope');
                                    return scopeOfOption[opsSelectCtrl.valueIdentifier] || scopeOfOption[opsSelectCtrl.keyIdentifier];
                                } else {
                                    return opsOption.attr('data-value');
                                }
                            }

                        }

                        function getOptionText(opsOption) {
                            var item = opsOption.find('a');
                        ...
                        function updateButtonContent() {
                            ...
                            if (isMultiple && modelValue.length === 0) {
                                ...
                                $timeout(function () {
                                    ...
                                    for (index = 0; index < length; index++) {
                                        opsOption = bsOptionElements.eq(index);
                                        if (opsOption.hasClass('nya-bs-option')) {

                                            value = getOptionValue(opsOption);

                                            if (isMultiple) {
                                                if (Array.isArray(modelValue) && contains(modelValue, value)) {
                                                    // if option has an label attribute. use the label value as content show in button.
                                                    // otherwise get very first child element.
                                                    optionLabel = opsOption.attr('label');
                                                    if (optionLabel) {
                                                        selection.push(document.createTextNode(optionLabel));
                                                    } else {
                                                        selection.push(getOptionText(opsOption));
                                                    }
    ...
    opsSelect.directive('opsOption', ['$parse', function ($parse) {
        ...
        return {
            ...
            priority: 1000,
            terminal: true,
            require: ['^opsSelect', '^ngModel'],
            compile: function opsOptionCompile(tElement, tAttrs) {
                var expression = tAttrs.opsOption;
                var opsOptionEndComment = document.createComment(' end opsOption: ' + expression + ' ');
                ...
                return function opsOptionLink($scope, $element, $attr, ctrls, $transclude) {

                    var opsSelectCtrl = ctrls[0],
                        ngCtrl = ctrls[1],
                        valueExpFn,
                        valueExpLocals = {};
                    ...
                    // set keyIdentifier and valueIdentifier property of opsSelectCtrl
                    if (keyIdentifier) {
                        opsSelectCtrl.keyIdentifier = keyIdentifier;
                    }
                    if (valueIdentifier) {
                        opsSelectCtrl.valueIdentifier = valueIdentifier;
                    }

                    if (valueExpGetter) {
                        opsSelectCtrl.valueExp = valueExp;
                    ...
                    if ($attr.deepWatch === 'true') {
                        $scope.$watch(collectionExp, opsOptionAction, true);
                    } else {
                        $scope.$watchCollection(collectionExp, opsOptionAction);
                    }

                    function opsOptionAction(collection) {
                        var index,
                        ...
                        for (index = 0; index < collectionLength; index++) {
                            ...
                            if (block.scope) {
                                ...
                                $transclude(function opsOptionTransclude(clone, scope) {
                                    ...
                                    var endNode = opsOptionEndComment.cloneNode(false);
                        ...
                        opsSelectCtrl.onCollectionChange(values);
                    }
                };
            }
        }
